## About Me

I'm a backend developer at [Reloshare](https://reloshare.com).

I've also built or contributed to several production applications using a modern Typescript stack and worked for [big](https://graybar.com) & [small](https://1904labs.com) companies as a data engineer & full stack developer. Check out my [LinkedIn](https://www.linkedin.com/in/samuel-phelan) for more information on my professional experience.

## Skills

- **Languages**: TypeScript, Javascript, HTML, CSS, SQL, Python
- **Infrastructure**: AWS, CloudFormation, Serverless Application Model
- **Frameworks**: React, Next.js, Express
- **Design Tools**: Figma, Adobe XD
- **Others**: Responsive design, Accessibility (a11y), Performance optimization

## Connect with Me

- **LinkedIn**: [in/samuel-phelan](https://www.linkedin.com/in/samuel-phelan)
- **Email**: [samphelan7@gmail.com](mailto:samphelan7@gmail.com)

Feel free to reach out if you're interested in collaborating or just want to chat about design and development!
